...
class AgentFactory {
  ...
  factory AgentFactory.create(
      {required String canisterId, required String url, required Service idl, Identity? identity, bool? debug = true}) {
    return AgentFactory(
        canisterId: canisterId, url: url, idl: idl, identity: identity ?? Ed25519KeyIdentity.generate(null), debug: debug);
  }
  ...
  void _initAgent(String url) async {
    ...
    _agent = HttpAgent(
        defaultProtocol: protocol, defaultHost: host, defaultPort: port, options: HttpAgentOptions()..identity = _identity);
    if (_debug) {
  ...
  void _createActor() {
    _actor = Actor.createActor(_idl, ActorConfig.fromMap({"canisterId": _canisterId, "agent": _agent}));
  }
}
